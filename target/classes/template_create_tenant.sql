...
SELECT a.jsonb->>'id' AS id,
  ...
  INNER JOIN myuniversity_mymodule.owners o ON o.jsonb->>'id' = f.jsonb->>'ownerId'
  LEFT JOIN diku_mod_circulation_storage.loan l ON a.jsonb->>'loanId' = l.jsonb->>'id'
  INNER JOIN diku_mod_inventory_storage.item i ON i.jsonb->>'id' = a.jsonb->>'itemId'
  INNER JOIN diku_mod_inventory_storage.material_type m ON i.jsonb->>'materialTypeId' = m.jsonb->>'id'
  INNER JOIN diku_mod_inventory_storage.holdings_record h ON i.jsonb->>'holdingsRecordId' = h.jsonb->>'id'
  INNER JOIN diku_mod_inventory_storage.instance ins ON ins.jsonb->>'id' = h.jsonb->>'instanceId'
) t;

--View item information
CREATE VIEW myuniversity_mymodule.item_information_view AS  SELECT id AS id, to_jsonb(t) AS jsonb
FROM (
  SELECT h.jsonb->>'id' AS "id",
